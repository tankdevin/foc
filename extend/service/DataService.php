...
class DataService
{
    ...
    public static function  save($dbQuery, $data, $key = 'id', $where = [])
    {
        ...
        if (Db::name($table)->where($where)->where($map)->count() > 0) {
            return Db::name($table)->strict(false)->where($where)->where($map)->update($data) !== false;
        }
        return Db::name($table)->strict(false)->insert($data) !== false;
    }


    public static function  other_save($dbQuery, $data, $where = [])
    {
        $db = is_string($dbQuery) ? Db::name($dbQuery) : $dbQuery;
        $table = $db->getTable();
        if (Db::name($table)->where($where)->count() > 0) {
            return Db::name($table)->strict(false)->where($where)->update($data) !== false;
        }
        return Db::name($table)->strict(false)->insert($data) !== false;
    }
    ...
    public static function update(&$dbQuery, $where = [])
    {
        ...
        if ($field === 'delete') {
            if (method_exists($db, 'getTableFields') && in_array('is_deleted', $db->getTableFields())) {
                return Db::name($table)->where($where)->where($map)->update(['is_deleted' => '1']) !== false;
            }
            return Db::name($table)->where($where)->where($map)->delete() !== false;
        }
        // 更新模式，更新指定字段内容
        return Db::name($table)->where($where)->where($map)->update([$field => $value]) !== false;
    }
