...
/*****************************************************************
Snoopy - 一个PHP版本的浏览器客户端，可以模拟用户的浏览操作。
作者: Monte Ohrt <monte@ispi.net>
版本: 1.2.4
本程序基于GNU版权协议，你可以自由的复制、开发和使用。
最新开源版下载地址：http://snoopy.sourceforge.net

此版本由国内的PHP爱好者修正或者丰富了一些功能。
以下列举一些贡献者 ：
1，尘缘，<admin@4wei.cn>，请不要来信，因为我有博客：www.4wei.cn。
*****************************************************************/
...
class Snoopy
{
	...
	var $strcookies		=	'';					// array of cookies to pass
...
\*======================================================================*/
	...
	function _httprequest($url,$fp,$URI,$http_method,$content_type="",$body="")
	{
		...
		if(!empty($this->cookies) || !empty($this->strcookies))
		{			
			if(!is_array($this->cookies))
				$this->cookies = (array)$this->cookies;
				
			if(trim($this->strcookies,"; \t")) $this->strcookies = 'Cookie: ' . trim($this->strcookies,"; \t");
				else $this->strcookies = '';
				
			reset($this->cookies);
			if ( count($this->cookies) > 0 ) {
				$cookie_headers .= $this->strcookies ? $this->strcookies . '; ' : 'Cookie: ';
				foreach ( $this->cookies as $cookieKey => $cookieVal ) {
					$cookie_headers .= $cookieKey."=".urlencode($cookieVal)."; ";
				}
				$headers .= substr($cookie_headers,0,-2) . "\r\n";
			}else{
				$this->strcookies && $headers .=  $this->strcookies . "\r\n";
			} 
		}
...
\*======================================================================*/
	...
	function getHttpHeader($info='filename')
	{
		if($info == 'filename') $info = "\s+filename=";
			else $info = trim($info , ":\t") . ': ';
		is_array($this->headers) && $this->headers = join("\n", $this->headers);
		preg_match("@$info([\s\S]*?)\n@is", $this->headers, $header);
		return trim($header[1]) ? trim($header[1]) : false;
	}
