...
namespace app\financial\service;
...
class OrderService
{
    ...
    public static function buildOrderList(&$list)
    {
        ...
        foreach ($list as $key => $vo) {
            ...
            $address_info = Db::name('store_member_address')->where('id',$vo['address_id'])->find();
            $address_order = getByIdAddress($address_info['province']).getByIdAddress($address_info['city']).getByIdAddress($address_info['area']).$address_info['address'];
            $address_info['address_detail'] = $address_order;
            $list[$key]['front_address'] = $address_info;
        }
        return $list;
    }
