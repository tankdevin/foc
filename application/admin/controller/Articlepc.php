 ...
 */
class Articlepc extends BasicAdmin
{
    ...
    public function edit()
    {
        ...
        if (request()->isPost()) {
            ...
            if ($id) {
                ...
                $res = Db::name("pc_article")->where(array(
                    'id' => $id
                ))->update($where);
                //执行入操作员操作日志表
                ...
                $where['img'] = $data['img'];
                $where['adminid'] = session('user')['id'];
                $res = Db::name("pc_article")->insertGetId($where);
                //执行入操作员操作日志表
        ...
        $info = [];
        if ($id) {
            $info = Db::name("pc_article")->where(array(
                'id' => $id
            ))->find();
        }
        ...
        if (request()->isPost()) {
            ...
            switch (strtolower($method)) {
                ...
                case  'forbid':
                    ...
                    break;
                case  'delete':
                    $res = Db::name('pc_article')->where($where)->delete();
                    if ($res) {
                        $this->success('删除成功', '');
                    } else {
                        $this->error('删除失败');
            ...
            if (Db::name('pc_article')->where($where)->update($data)) {
                $this->success('操作成功', '');
            } else {
                $this->error('操作失败');
            }
