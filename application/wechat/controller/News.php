...
class News extends BasicAdmin
{
    ...
    public function not_bind(){
        $data = Db::name('store_member')->where('openid',null)->select();
        foreach($data as $k=>$v){
            $data[$k]['first_parent_info'] = Db::name('store_member')->where(['id'=>$v['first_leader']])->find();
            $data[$k]['member_level_title'] = Db::name('sys_level')->where(['id'=>$v['member_level']])->value('title');
        }
