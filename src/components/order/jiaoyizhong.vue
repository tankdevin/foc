<template>
  <div class="content">
    ...
    <div class="txt1">{{info.order_info.interval_info}}订单交易中</div>
    <div class="txt2" v-if="info.order_info.status != 4">交易倒计时</div>
    <div class="txt2" v-if="info.order_info.status != 4 ">{{hour || '00' }}时{{minute || '00'}}分{{second || '00'}}秒</div>
    ...
    <div class="main">
      ...
      <div class="box4" v-if="info.order_info.type == 1">
        <p>付款凭证</p>
        <p>
    ...
    <div class="send" @click="fukuan" v-if="info.order_info.type == 1">确认付款</div>
    <div class="send" @click="shoukuan" v-if="info.order_info.type == 2">确认收款</div>
    <!--<div class="cancel">取消购买</div>-->

    <div class="mask" v-if="wx_boon" @click="close_win"></div>
    <div class="wx" v-if="wx_boon" @click="close_win">
<!--      <p>{{info.payment_wexin.payment}}</p>-->
      <img :src="info.payment_wexin.paymentimg" alt="">
    </div>

    <div class="mask" v-if="al_boon" @click="close_win"></div>
    <div class="wx" v-if="al_boon" @click="close_win">
        ...
        methods: {
          ...
          fukuan(){//买方确认付款
            ...
            if(that.imgs == ''){
              ...
              that.$post('/apiv1/index/confrimFukuan',{
                image: that.imgs,
                orderid: that.id
              }).then(res=>{
                that.$msg(res.msg)
