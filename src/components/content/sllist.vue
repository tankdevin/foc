<template>
  <div class="content" v-swipeup="{fn:vuetouch,name:'上滑'}">
    ...
    <div class="main">
      ...
      <div class="list1" v-for="(i,index) in list" @touchmove="touched(index,i.status)"  v-swipeleft="{fn:vuetouch,name:'左滑'}"  v-swiperight="{fn:vuetouch,name:'右滑'}">
        <p>{{i.create_at}}</p>
        <p>{{i.bonus_total*1}}</p>
        <p>{{i.real_price*1}}</p>
        ...
        <div v-if="touch_index1 == index" @click="cencel(i)">取消</div>
      </div>
    </div>
