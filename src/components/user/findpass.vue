<template>
  <div class="content">
    ...
    <div class="main">
      ...
      <div class="box1">
        <p>确认密码</p>
        <input type="password" placeholder="请确认新密码" v-model="info.repassword">
      </div>
      <div class="send" @click="send">确认</div>
    </div>
        ...
        methods: {
          ...
          sendcode(){
            ...
            if(this.info.phone == ''){
              ...
              if(that.nums == 60){
                this.$post('/apiv1/login/sendMobileMessage',{phone: this.info.phone,type:'forget'}).then(res => {
          ...
          send(){
            ...
            if(that.info.phone == ''){
              ...
              that.$msg('请重复新密码')
            }else if(that.info.repassword != that.info.password){
              that.$msg('两次密码不一致')
            }else{
              that.$post('/apiv1/login/forget',that.info).then(res=>{
                that.$msg(res.msg)
                if(res.code == 1){
                  that.$router.go(-1)
                }
